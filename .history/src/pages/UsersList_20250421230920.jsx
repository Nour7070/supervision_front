        ...
        const fetchUsers = async () => {
            try {
                ...
                if (userType === "FORMATEUR") {
                    // Utilise le nouvel endpoint pour récupérer seulement les formateurs approuvés
                    response = await axios.get("/api/users/db/formateurs/approved");
                } else {
                    // Pour les autres types (ex: APPRENANT), utilise l’endpoint générique
                    response = await axios.get(`/api/users/db/${userType}`);
                }
    ...
    if (loading) {
        return (
            <div className="flex justify-center items-center h-64">
                <div className="animate-pulse text-teal-600">Chargement en cours...</div>
            </div>
        );
    }
                                            ...
                                            {currentItems.map(user => (
                                                <tr key={user.id} className="hover:bg-gray-50 transition-colors">
                                                    ...
                                                    <td className="px-6 py-4 whitespace-nowrap">
                                                        <button
                                                            onClick={() => {
                                                                handleViewStats(user.id);
                                                                navigate(`/users/${user.role.toLowerCase()}/${user.id}/stats`);
                                                            }}
                                                            
                                                            className="px-3 py-1 bg-gray-200 text-black rounded-md hover:bg-[#938A8A] transition-colors flex items-center gap-1"
                                                        >
