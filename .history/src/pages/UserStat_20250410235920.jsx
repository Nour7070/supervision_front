...
const UserStat = () => {
    ...
    useEffect(() => {
        setTimeout(() => {
            ...
            // Complétez avec des valeurs par défaut si nécessaire
            setStats({
                ...userData,
            ...
            setLoading(false);
        }, 500); // Réduisez le délai de chargement simulé
    }, [id, userType]);
        ...
        <div className="container mx-auto p-6">
            ...
            <div className="flex flex-col md:flex-row items-start md:items-center mb-8 gap-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <p><strong className="text-gray-700">Âge :</strong> {stats.age} ans</p>
                    <p><strong className="text-gray-700">Date d'intégration :</strong> {stats.dateIntegration}</p>
                    <p><strong className="text-gray-700">ID Utilisateur :</strong> {id}</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {userType === 'FORMATEUR' ? (
                    <>
                        <StatCard title="Cours publiés" value={stats.coursPublies} />
                        <StatCard title="Cours en attente" value={stats.coursEnAttente} />
                        <StatCard title="Étudiants abonnés" value={stats.etudiantsAbonnes} />
                        <StatCard title="Quizz publiés" value={stats.quizzPublies} />
                    </>
                ) : (
                    <>
                        <StatCard title="Cours suivis" value={stats.coursSuivis} />
                        <StatCard title="Cours terminés" value={stats.coursTermines} />
                        <StatCard title="Quizz réussis" value={stats.quizzReussis} />
                        <StatCard title="Quizz échoués" value={stats.quizzEchoues} />
                    </>
                )}
            </div>
        </div>
