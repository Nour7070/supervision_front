...
const formatTimeAgo = (timestamp) => {
 ...
 const maxValue = Math.max(
  ...
  // Pour les PDF et images - afficher dans un nouvel onglet
  if (['pdf', 'jpg', 'jpeg', 'png', 'gif'].includes(extension)) {
    window.open(`http://localhost:8081/api/file-content/${encodeURIComponent(filePath)}`, '_blank');
  } 
  // Pour tous les autres types (y compris PPTX) - forcer le téléchargement
  else {
    const link = document.createElement('a');
    link.href = `http://localhost:8081/api/file-content/${encodeURIComponent(filePath)}`;
    link.download = filePath.split('/').pop(); // Force le téléchargement avec le nom original
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  }
