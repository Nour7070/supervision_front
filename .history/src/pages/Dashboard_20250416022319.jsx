...
const Dashboard = ({ userType }) =>
{
    ...
    useEffect(() => {
        const fetchStats = async () => {
            setLoading(true);
            try {
                // 1. Requête pour les stats
                const statsResponse = await axios.get('/api/dashboard/stats');
                const { formateurs, apprenants, moderateurs } = statsResponse.data;

                // 2. Requête pour les formateurs en attente
                const pendingFormateurs = await axios.get('/supervision/formateurs/pending');

                // 3. Requête pour les cours en attente avec meilleure gestion d'erreur
                let pendingCourses = [];
                try {
                    const pendingCoursesResponse = await axios.get('/courses/pending');
                    
                    // Gestion des différents formats de réponse
                    if (Array.isArray(pendingCoursesResponse.data)) {
                        pendingCourses = pendingCoursesResponse.data;
                    } else if (pendingCoursesResponse.data && Array.isArray(pendingCoursesResponse.data.data)) {
                        pendingCourses = pendingCoursesResponse.data.data;
                    } else if (typeof pendingCoursesResponse.data === 'string') {
                        pendingCourses = JSON.parse(pendingCoursesResponse.data);
                    }
                } catch (error) {
                ...
                console.log("Données reçues - Formateurs:", pendingFormateurs.data);
                ...
                setStats({
                    ...
                    pendingCourseSubmissions: pendingCourses.map(c => ({
                        id: c.id,
                        formateur: c.formateurNom || `Formateur ${c.formateurId}`,
                        formateurId: c.formateurId,
                        title: c.titre || c.coursNom || "Sans titre",
                        description: c.description || "Description non fournie",
                        file: c.chapters?.[0]?.fileUrl || "#",
                        additionalFiles: "#",
                        duration: "N/A",
                        price: "N/A",
                        date: c.dateCreation || new Date().toISOString().split('T')[0]
                    }))
                });
            } catch (error) {
