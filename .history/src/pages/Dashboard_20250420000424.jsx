...
const Dashboard = ({ userType }) => {
    ...
    const handlePreviewChapter = async (fileUrl) => {
        try {
          // 1. Utilisez le chemin relatif si c'est un fichier uploadé
          const apiUrl = fileUrl.includes('http') ? fileUrl : 
                        `/api/file-content/chapter?fileUrl=${encodeURIComponent(fileUrl)}`;
          
          // 2. Pour les PDF, ouvrez directement dans un nouvel onglet
          if (fileUrl.endsWith('.pdf')) {
            window.open(apiUrl, '_blank');
          } 
          // 3. Pour les autres fichiers, utilisez l'API
          else {
