        ...
        const fetchStats = async () =>
        {
            ...
            try
            {
                ...
                try {
                    ...
                    // Solution 1: Si la réponse est une string JSON
                    ...
                    // Solution 2: Si la réponse est un objet avec data
                    if (response.data && typeof response.data === 'object') {
                      // Gestion spéciale pour la structure récursive
                      if (Array.isArray(response.data)) {
                        return response.data.map(course => ({
                          ...course,
                          // Coupe la récursion en ne gardant que les infos nécessaires
                      ...
                      // Si les cours sont dans une propriété data
                      if (response.data.data) {
                        if (Array.isArray(response.data.data)) {
                          return response.data.data.map(course => ({
                            ...course,
                            chapters: course.chapters?.slice(0, 1) || [] // Ne garde que le premier chapitre
                          }));
                        } else if (typeof response.data.data === 'string') {
                          try {
                            return JSON.parse(response.data.data);
                          } catch (e) {
                            console.error("Erreur parsing data.data:", e);
                          }
                        }
                      }
                    }
