...
const Layout = ({ onLogout }) =>
{
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      ...
      <div className={`hidden md:flex md:flex-shrink-0 ${sidebarOpen ? 'w-60' : 'w-20'} bg-[#1A4D3E] transition-all duration-300 flex-col`}>
        ...
        <div className="flex-1 overflow-y-auto">
          <nav className="px-2 py-4" >
            <Link
              to="/dashboard"
              className="flex items-center bg-[#E8F4F0] px-4 py-3 text-indigo-100 hover:bg-white-600 rounded-lg group"
            >
      ...
      <div className={`fixed inset-y-0 left-0 z-20 w-64 bg-white-700 transform ${mobileSidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:hidden transition-transform duration-300`}>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className=" bg-[#1A4D3E] shadow-sm z-10">
          <div className="flex items-center justify-between h-16 px-4">
            <div className="flex items-center">
              <button
                onClick={() => setMobileSidebarOpen(true)}
                className="md:hidden text-white-500 p-1 rounded-md hover:bg-white-100"
              >
                <FiMenu size={24} />
              </button>
              <button
                onClick={() => setSidebarOpen(!sidebarOpen)}
                className="hidden md:block ml-2 text-white-500 p-1 rounded-md hover:bg-white-100"
              >
                <FiMenu size={24} />
              </button>
            ...
            <div className="flex items-center space-x-4">
              <button className="p-1 rounded-full text-white-400 hover:text-white-500 focus:outline-none focus:ring-2 focus:ring-indigo-500">
                <FiBell size={20} />
              </button>
              <div className="flex items-center">
