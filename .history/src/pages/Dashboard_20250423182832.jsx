...
const formatTimeAgo = (timestamp) => {
 ...
 const maxValue = Math.max(
    ...
    /*const handlePreviewFile = (filePath) => {
      ...
      const handlePreviewFile = (filePath) => {
  // Pour les PDF, JPG, etc. - afficher dans un nouvel onglet
  if (['pdf', 'jpg', 'jpeg', 'png', 'gif'].includes(filePath.split('.').pop().toLowerCase())) {
    window.open(`http://localhost:8081/api/file-content/${encodeURIComponent(filePath)}`, '_blank');
  } 
  // Pour les PPTX - également ouvrir dans un nouvel onglet
  else if (['pptx', 'ppt'].includes(filePath.split('.').pop().toLowerCase())) {
    window.open(`http://localhost:8081/api/file-content/${encodeURIComponent(filePath)}`, '_blank');
  }
