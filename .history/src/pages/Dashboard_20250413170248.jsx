...
const Dashboard = ({ userType }) =>
{
    ...
    // Données simulées pour les sections sans endpoint
    const data = [
    ...
    useEffect(() =>
    {
        const fetchStats = async () =>
        {
            ...
            try
            {
                ...
                const { formateurs, apprenants, quizz, moderateurs } = statsResponse.data;

                // Fetch pending trainers
                const pendingFormateurs = await axios.get('/supervision/formateurs/pending');

                // Fetch pending courses
                const pendingCourses = await axios.get('/validations');
                console.log("pendingFormateurs:", pendingFormateurs.data);
                ...
                setStats({
                    ...
                    pendingRegistrations: pendingFormateurs.data.map(f => ({
                        id: f.id,
                        name: `${f.prenom} ${f.nom}`,
                        email: f.email,
                    ...
                    pendingCourseSubmissions: pendingCourses.data.map(c => ({
                        id: c.id,
                        formateur: "Nom du formateur", // À adapter si nécessaire
    ...
    const handleApproveRegistration = async (id) =>
    {
        try
        {
            await axios.put(`/supervision/formateurs/${id}/approve`);
            setStats(prev => ({
                ...prev,
    ...
    const handleRejectRegistration = async (id) =>
    {
        try
        {
            await axios.put(`/supervision/formateurs/${id}/reject`);
            setStats(prev => ({
                ...prev,
                pendingRegistrations: prev.pendingRegistrations.filter(req => req.id !== id)
            }));
    ...
    const handleApproveCourse = async (id) =>
    {
        try
        {
            await axios.put(`/validations/${id}/approve`);
            setStats(prev => ({
                ...prev,
    ...
    const handleRejectCourse = async (id) =>
    {
        try
        {
            await axios.put(`/validations/${id}/reject`);
            setStats(prev => ({
                ...prev,
                pendingCourseSubmissions: prev.pendingCourseSubmissions.filter(course => course.id !== id)
            }));
    ...
    return (
        <div className="p-6 bg-gray-50 min-h-screen">
            ...
            <h2 className="text-sm font-semibold mb-2">Utilisateurs par type</h2>
            <div className="space-y-2">
                <div className="flex items-center">
                    <div className="w-24 text-xs text-gray-600">Formateurs</div>
                    <div className="w-full max-w-xs bg-gray-200 rounded-full h-2">
                        <div
                            className="bg-blue-600 h-2 rounded-full"
                            style={{ width: `${calculatePercentage(stats.totalFormateurs || 0)}%` }}
                        ></div>
                    </div>
                    <div className="ml-2 text-xs font-medium">{stats.totalFormateurs || 0}</div>
                </div>
                
                <div className="flex items-center">
                    <div className="w-24 text-xs text-gray-600">Apprenants</div>
                    <div className="w-full max-w-xs bg-gray-200 rounded-full h-2">
                        <div
                            className="bg-green-600 h-2 rounded-full"
                            style={{ width: `${calculatePercentage(stats.totalApprenants || 0)}%` }}
                        ></div>
                    </div>
                    <div className="ml-2 text-xs font-medium">{stats.totalApprenants || 0}</div>
                </div>
                
                {userType?.toUpperCase() === 'SUPERVISEUR' && (
                    <div className="flex items-center">
                        <div className="w-24 text-xs text-gray-600">Modérateurs</div>
                        <div className="w-full max-w-xs bg-gray-200 rounded-full h-2">
                            <div
                                className="bg-purple-600 h-2 rounded-full"
                                style={{ width: `${calculatePercentage(stats.totalModerateurs || 0)}%` }}
                            ></div>
                        </div>
                        <div className="ml-2 text-xs font-medium">{stats.totalModerateurs || 0}</div>
                    </div>
                )}
            </div>
