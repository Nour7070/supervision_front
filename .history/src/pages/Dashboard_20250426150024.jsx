...
/*const handlePreviewFile = (fileUrl) => {
  ...
  <div className="space-y-4 h-[136px] overflow-y-auto pr-2"> 
      ...
      recentActivities.map((activity, index) => (
        <div key={index} className="flex items-center mb-2 min-h-[60px]">
          <div className={`w-6 h-6 rounded-full mr-4 flex justify-center items-center ${
            ...
            activity.type === 'NEW_COURSE_TO_VALIDATE' ? 'bg-blue-500' :
            activity.type === 'NEW_STUDENT_SIGNUP' ? 'bg-yellow-500' : 
            'bg-gray-400'
          }`}>
            <i className={`fas ${
  ...
  <div className="relative">
    <div className="h-[272px] overflow-y-auto"> {/* 4 éléments max visibles (68px chacun) */}
      <AnimatePresence>
    ...
    {/* Cache les éléments supplémentaires visuellement */}
...
<div className="bg-white p-4 rounded-lg shadow-sm">
  ...
  <div className="space-y-3 h-[272px] overflow-y-auto pr-2"> {/* Hauteur augmentée pour 4 éléments */}
    <AnimatePresence>
      <motion.div className="space-y-3" {...animationProps}>
        {stats.pendingCourseSubmissions.slice(0, 4).map((course) => (
