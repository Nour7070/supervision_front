    ...
    useEffect(() =>
    {
        const fetchStats = async () =>
        {
            ...
            try
            {
                ...
                try
                {
                    const response = await axios.get('/courses/pending');
                    console.log("Réponse brute du backend:", response.data);

                    // Vérifier simplement ce que nous recevons et extraire les cours
                    if (Array.isArray(response.data))
                    {
                        pendingCourses = response.data;
                    } else if (response.data && typeof response.data === 'object')
                    {
                        // Si la réponse contient un tableau 'data'
                        if (Array.isArray(response.data.data))
                        {
                            pendingCourses = response.data.data;
                        }
                        // Si nous avons directement un cours
                        else if (response.data.id && response.data.titre)
                        {
                            pendingCourses = [response.data];
                        }
                    }

                    console.log("Cours en attente après extraction:", pendingCourses);
                } catch (error)
                {
                ...
                setStats({
                    ...
                    pendingCourseSubmissions: pendingCourses.map(c => ({
                        id: c.id,
                        formateur: c.formateur?.nom || `Formateur ${c.formateurId}`,
                        formateurId: c.formateurId,
                        title: c.titre || c.nom || "Sans titre",
                        description: c.description || "Description non fournie",
                        file: c.fichierUrl || c.chapters?.[0]?.fileUrl || "#",
                        additionalFiles: "#",
                        duration: "N/A",
                        price: "N/A",
