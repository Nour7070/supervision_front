...
*/
function App() {
  const [userType, setUserType] = useState('MODERATEUR'); // À remplacer par votre vrai état d'authentification

  return (
    <Router>
      <Routes>
        <Route path="/" element={<Layout userType={userType} onLogout={() => { }} />}>
          {/* Route conditionnelle pour le dashboard */}
          <Route 
            path="/dashboard" 
            element={
          ...
          {/* Routes des utilisateurs - accessibles à tous */}
          <Route path="users">
            <Route path="formateur" element={<UsersList userType="FORMATEUR" />} />
            <Route path="apprenant" element={<UsersList userType="APPRENANT" />} />
          </Route>

          {/* Routes des modérateurs - seulement pour SUPERVISEUR */}
          {userType === 'SUPERVISEUR' && (
            <Route path="moderateurs">
              <Route index element={<ModerateursListe userType={userType} />} />
              <Route path="add" element={<AddModerateur />} />
          ...
          {/* Route des statistiques modérateur - seulement pour SUPERVISEUR */}
          {userType === 'SUPERVISEUR' && (
            <Route path="/moderateurs/:id/statistiques" element={<Dashboard />} />
          )}
