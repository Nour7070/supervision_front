...
/*const handlePreviewFile = (fileUrl) => {
  ...
  {/* Trainer Requests Section - Ajout d'un cadre */}
  ...
  {/* Course Requests Section - Ajout d'un cadre */}
  <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
    <div className="flex justify-between items-center mb-3">
...
{/* Moderator Actions Section - Déplacée après la grille et ajout d'un cadre */}
{userType?.toUpperCase() === 'SUPERVISEUR' && (
  <div className="mt-6 bg-white p-4 rounded-lg shadow-sm border border-gray-200">
    <div className="flex justify-between items-center mb-3">
      <h2 className="text-base font-semibold">Moderator Actions Log</h2>
    </div>
    <div className="relative">
      <div className="h-[272px] overflow-y-auto scrollbar scrollbar-thumb-[#034732] scrollbar-track-gray-100 scrollbar-thumb-rounded-full pr-5">
        {moderatorActions.length > 0 ? (
          moderatorActions.map((action, index) => (
            <div key={index} className="border-b pb-3 last:border-0">
              <div className="flex items-start">
                <div className={`w-8 h-8 rounded-full mr-3 flex items-center justify-center mt-1
                  ${action.type.includes('APPROVAL') ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'}`}>
                  {action.type.includes('APPROVAL') ? '✓' : '✗'}
                </div>
                <div>
                  <p className="font-medium">{action.description}</p>
                  <p className="text-gray-500 text-sm">
                    {formatTimeAgo(action.timestamp)} • By {action.moderatorName}
                  </p>
                </div>
              </div>
            </div>
          ...
          <p className="text-gray-500 text-sm text-center py-4">No moderator actions yet</p>
        )}
      </div>
      <div className="absolute bottom-0 left-0 right-0 h-6 bg-gradient-to-t from-white to-transparent pointer-events-none"></div>
    </div>
