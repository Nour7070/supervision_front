...
const ModerateursListe = ({ userType }) =>
{
  ...
  useEffect(() =>
  {
    const fetchModerateurs = async () =>
    {
      try
      {
        // Simuler des données pour les modérateurs
        const simulatedModerateurs = [
          { id: 1, nom: 'John Doe', email: 'john@example.com', permissions: ['View', 'Edit'] },
          { id: 2, nom: 'Jane Smith', email: 'jane@example.com', permissions: ['View'] },
          { id: 3, nom: 'Alice Johnson', email: 'alice@example.com', permissions: ['View', 'Delete'] },
        ];
  ...
  if (loading) return <p className="text-center text-gray-500">Chargement des modérateurs...</p>;
    ...
    <div className="container mx-auto p-6">
      ...
      <div className="flex justify-between mb-6">
        <input
          ...
          className="border border-gray-300 rounded-lg p-2 w-1/2 focus:outline-none focus:ring-2 focus:ring-teal-500"
        />
        <input
          type="text"
          value={`Nombre total des modérateurs : ${filteredModerateurs.length}`}
          className="border border-gray-300 rounded-lg p-2 w-1/4 text-center bg-gray-100 cursor-default"
          disabled
        />
      </div>
      ...
      {contextMenu && (
        <div
          className="context-menu absolute bg-white border border-gray-300 shadow-lg rounded-lg"
