...
/*const handlePreviewFile = (fileUrl) => {
  ...
  const handlePreviewFile = (fileUrl) => {
    ...
    if (['pdf', 'jpg', 'jpeg', 'png', 'gif'].includes(extension)) {
        // Ouvrir directement les PDF et images
        window.open(`http://localhost:8081/api/file-content/${encodeURIComponent(fileName)}`, '_blank');
    } else if (extension === 'pptx') {
        // Pour les PPTX, utiliser l'API de téléchargement
        window.open(`http://localhost:8081/api/file-download/${encodeURIComponent(fileName)}`, '_blank');
    } else {
        ...
        .then((response) => {
            ...
            const link = document.createElement('a');
            link.href = url;
            link.setAttribute('download', fileName); // Utiliser fileName au lieu de filePath
            document.body.appendChild(link);
            link.click();
            link.remove();
            window.URL.revokeObjectURL(url);
