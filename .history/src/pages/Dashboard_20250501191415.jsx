...
import backgroundPattern from "../../public/images/Screenshot_2181.jpg";
...
/*const handlePreviewFile = (fileUrl) => {
    // Extraire uniquement le nom du fichier, quelle que soit la façon dont l'URL est formatée
    const fileName = fileUrl.includes('/') ? fileUrl.split('/').pop() : fileUrl;
    const extension = fileName.split('.').pop().toLowerCase();
    
    console.log(`Ouverture du fichier: ${fileName} (extension: ${extension})`);
    
    // En fonction du type de fichier, on utilise la route API appropriée
    if (['pdf', 'jpg', 'jpeg', 'png', 'gif'].includes(extension)) {
        // Pour les fichiers à afficher directement dans le navigateur (PDF, images)
        window.open(`/api/file-content/${encodeURIComponent(fileName)}`, '_blank');
    } else {
        // Pour tous les autres types de fichiers, on propose un téléchargement
        // Utiliser la route relative pour s'adapter à l'environnement
        axios({
            url: `/files/api/file-download/${encodeURIComponent(fileName)}`,
            method: 'GET',
            responseType: 'blob',
        })
        .then((response) => {
        ...
        .catch((error) => {
            console.error('Erreur lors du téléchargement du fichier:', error);
            // En cas d'erreur, tenter une approche alternative
         ...
         <div className="fixed inset-0 z-0 bg-cover bg-center" 
          ...
          <div className="relative z-10 p-6 space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              <div className="bg-white rounded-xl shadow-lg p-6 flex items-start h-full border border-gray-400">
                <div className="bg-blue-100 p-3 rounded-full mr-4">
                  <i className="fas fa-user-check text-blue-500 text-xl"></i>
                </div>
                <div>
              ...
              <div className="bg-white rounded-xl shadow-lg p-6 flex items-start h-full">
                <div className="bg-green-100 p-3 rounded-full mr-4">
                  <i className="fas fa-graduation-cap text-green-500 text-xl"></i>
                </div>
                <div>
            ...
            <span className="text-gray-500 text-sm">
              ...
              <div className="bg-white rounded-xl shadow-lg p-6 border border-gray-400">
                ...
                <div className="space-y-4">
                  ...
                  {userType?.toUpperCase() === 'SUPERVISEUR' && (
                    <div className="flex items-center ">
                      <div className="w-1/3 text-sm text-gray-600 pr-3">Moderators</div>
                      <div className="w-full bg-gray-200 rounded-full h-3">
                        <div
                          className="bg-purple-600 h-3 rounded-full"
                          style={{ width: `${calculatePercentage(stats.totalModerateurs)}%` }}
