    ...
    <div className="flex min-h-screen bg-gray-50">
        ...
        <nav>
          <ul className="space-y-2">
            {['Home', 'Profile', 'Settings', 'Messages', 'Courses'].map(item => (
              <li key={item}>
                <button
                  className="w-full text-left px-4 py-3 rounded hover:bg-blue-700 transition-colors"
                >
                  {item}
                </button>
              </li>
      ...
      <main className="flex-1 ml-64 p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          {[
            { title: 'Total Users', value: 120, bg: 'bg-blue-100', text: 'text-blue-800' },
            { title: 'Active Courses', value: 8, bg: 'bg-green-100', text: 'text-green-800' },
            { title: 'New Messages', value: 5, bg: 'bg-purple-100', text: 'text-purple-800' }
          ].map((stat, index) => (
            <div
              key={index}
              className={`${stat.bg} ${stat.text} p-6 rounded-xl shadow transition-transform hover:scale-105`}
            >
