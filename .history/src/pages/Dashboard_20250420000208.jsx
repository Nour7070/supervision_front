...
const Dashboard = ({ userType }) => {
    ...
    const handlePreviewChapter = async (fileUrl) => {
        try {
          // 1. Nettoyez et encodez l'URL
          const cleanedFileUrl = fileUrl.startsWith('http') ? fileUrl : 
                               `file://${fileUrl.replace(/^\/+/, '')}`;
          
          const encodedFileUrl = encodeURIComponent(cleanedFileUrl);
      
          // 2. Appel API avec logging
          console.log("Envoi de la requête avec fileUrl:", encodedFileUrl);
          
          const response = await axios.get('/api/file-content/chapter', {
            params: { fileUrl: encodedFileUrl },
            headers: {
          ...
          if (response.data && response.data.content) {
            setPreviewContent({
              title: response.data.fileName.split('/').pop(),
              text: response.data.content
            });
          } else {
            throw new Error("Contenu non disponible dans la réponse");
          }
          ...
          alert(`Impossible de prévisualiser: ${error.message}. Le fichier s'ouvre dans un nouvel onglet.`);
        }
      };
