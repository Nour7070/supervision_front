...
const Maps = () => {
  return (
    <ComposableMap>
      <Geographies geography="path_to_your_geojson_file.json">
        {({ geographies }) =>
          geographies.map((geo) => (
...
// Fonction pour générer une échelle de couleurs à partir de #034732
const generateColorScale = (baseColor) => {
    return scaleSequential()
        .domain([0, 5000])
        .interpolator(interpolate)
        .range(['#f0f9e8', baseColor]); // Du vert très clair au vert foncé
};

const Maps = () => {
    const baseColor = '#034732';
    const colorScale = generateColorScale(baseColor);

    // Trouver le pays avec le plus d'utilisateurs pour la légende
            ...
            <div className="w-full" style={{ aspectRatio: '2 / 1' }}>
                ...
                >
                    <ZoomableGroup center={[0, 30]} zoom={1}>
                        ...
                        {/* Marqueurs pour les pays avec le plus d'utilisateurs */}
                        {Object.entries(countryData)
                            ...
                            .map(([code, users]) => {
                                ...
                                return (
                                    <Marker key={code} coordinates={[x, y]}>
                                        <circle
