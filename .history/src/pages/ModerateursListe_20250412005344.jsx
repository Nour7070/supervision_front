...
const ModerateursListe = ({ userType }) =>
{
  ...
  useEffect(() => {
    const fetchModerateurs = async () => {
      try {
        const response = await axios.get('http://localhost:8081/api/moderateurs');
        
        // Formatage des données pour correspondre à votre frontend
        const formattedData = response.data.map(moderateur => ({
          id: moderateur.id,
          nom: moderateur.prenom && moderateur.nom 
               ? `${moderateur.prenom} ${moderateur.nom}`
               : moderateur.username, // Fallback si nom/prénom manquants
