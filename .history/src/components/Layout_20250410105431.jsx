...
const Layout = ({ children }) => {
  ...
  // Icônes de remplacement (vous pouvez utiliser des SVG ou du texte)
  const icons = {
    home: '🏠',
    users: '👥',
    documents: '📄',
      ...
      <div className={`hidden md:flex md:flex-shrink-0 ${sidebarOpen ? 'w-64' : 'w-20'} bg-indigo-700 transition-all duration-300 flex-col`}>
        ...
        <div className="flex-1 overflow-y-auto">
          <nav className="px-2 py-4">
            {[
              { name: 'Accueil', icon: icons.home, href: '#' },
              { name: 'Utilisateurs', icon: icons.users, href: '#' },
              { name: 'Documents', icon: icons.documents, href: '#' },
              { name: 'Paramètres', icon: icons.settings, href: '#' },
            ].map((item) => (
              <a
                key={item.name}
                href={item.href}
