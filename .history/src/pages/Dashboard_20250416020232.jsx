...
const Dashboard = ({ userType }) =>
{
    ...
    useEffect(() =>
    {
        const fetchStats = async () =>
        {
            ...
            try
            {
                ...
                const pendingCourses = await axios.get('/courses/pending');
    ...
    const handleApproveRegistration = async (id) =>
    {
        try
        {
            await axios.put(`/courses/${id}/approve`);
            setStats(prev => ({
                ...prev,
    ...
    const handleRejectRegistration = async (id) =>
    {
        try
        {
           await axios.put(`/courses/${id}/reject`);
            setStats(prev => ({
                ...prev,
                pendingRegistrations: prev.pendingRegistrations.filter(req => req.id !== id)
            }));
...
const Dashboard = ({ userType }) => {
    ...
    useEffect(() => {
        const fetchStats = async () => {
            ...
            try {
                ...
                const pendingCourses = await axios.get('/api/moderateurs/pending');
                ...
                setStats({
                    ...
                    pendingCourseSubmissions: pendingCourses.data.map(c => ({
                        id: c.id,
                        formateur: c.formateurId, // À adapter selon vos besoins
                        formateurId: c.formateurId,
    ...
    const handleRejectRegistration = async (email) => {
        try {
            ...
            console.error("Erreur de rejet:", error.response?.data || error.message);
    ...
    const handleApproveCourse = async (courseId) => {
        try {
            await axios.put(`/api/moderateurs/${courseId}/approve`);
            setStats(prev => ({
                ...prev,
                pendingCourseSubmissions: prev.pendingCourseSubmissions.filter(course => course.id !== courseId),
            }));
        } catch (error) {
            console.error("Erreur d'approbation:", error.response?.data || error.message);
            alert(`Erreur: ${error.response?.data?.message || "Échec de l'approbation"}`);
    ...
    const handleRejectCourse = async (courseId) => {
        try {
            await axios.put(`/api/moderateurs/${courseId}/reject`);
            setStats(prev => ({
                ...prev,
                pendingCourseSubmissions: prev.pendingCourseSubmissions.filter(course => course.id !== courseId)
            }));
    ...
    // File preview modal (identique à votre version originale)
    const FilePreviewModal = () => (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            {/* ... contenu identique ... */}
        </div>
