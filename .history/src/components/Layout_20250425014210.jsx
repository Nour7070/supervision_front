...
const Layout = ({ onLogout }) =>
{
  ...
  useEffect(() => {
    ...
    // Pour les tests, si l'email n'est pas trouvé, utilisez un email de test
    const emailToUse = userEmail || 'test6@gmail.com';
    console.log("Using email:", emailToUse);
    
    axios.get(`/api/users/info?email=${encodeURIComponent(emailToUse)}`)
      .then(response => {
        console.log("User data received:", response.data);
        localStorage.setItem('userType', response.data.userType);
        setUserType(response.data.userType);
  ...
  console.log("Domaine complet :", window.location.host); // Ex: "auth.mondomaine.com:8080"
  console.log("Nom de domaine :", window.location.hostname); // Ex: "auth.mondomaine.com"
  return (
          ...
          <div className="flex items-center justify-between h-10 px-4">
            <div className="flex items-center">
              ...
              <div className="relative ml-4">
                ...
                <input
                  type="text"
                  className="block w-full pl-10 pr-3 py-1 w-56 border border-gray-300 rounded-md leading-5 bg-white placeholder-gray-500 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                  placeholder="Search..."
                />
              </div>
            </div>
