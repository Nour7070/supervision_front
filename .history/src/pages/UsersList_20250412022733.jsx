    ...
    useEffect(() => {
        const fetchUsers = async () => {
            try {
                ...
                try {
                    response = await axios.get(`api/users/db/${backendUserType}`);
                } catch (primaryError) {
                    console.warn("L'endpoint principal a échoué, tentative avec l'endpoint de secours");
                    response = await axios.get(`/users/db/${backendUserType}`);
                }

                // Formatage des données
                const formattedUsers = response.data.map(user => ({
                    id: user.id,
                    name: [user.prenom, user.nom].filter(Boolean).join(' ') || user.username,
                    email: user.email,
                    role: user.userType
                }));
    ...
    return (
        <div className="relative w-full min-h-screen overflow-hidden">
            ...
            <div className="absolute inset-0 z-0 bg-cover bg-center opacity-60" 
                 style={{ backgroundImage: 'url(/images/ph3.jpg)' }} />
            ...
            <div className="relative z-10 w-full h-full">
                <div className="max-w-6xl mx-auto">
                    ...
                    <div className="text-center pt-6">
                        <h1 className="text-2xl md:text-3xl font-bold text-gray-800">
                            {userType === "FORMATEUR" ? "Liste des Formateurs" : "Liste des Apprenants"}
                        </h1>
                        <p className="text-gray-500 mt-2">Gestion des accès</p>
                    </div>
                                        ...
                                        <tbody className="bg-white divide-y divide-gray-200">
                                            {filteredUsers
                                                ...
                                                <tr key={user.id} className="hover:bg-gray-50 transition-colors">
                                                    ...
                                                    <td className="px-6 py-4 whitespace-nowrap">
                                                        <button
                                                            onClick={() => navigate(`/users/${userType}/${user.id}/stats`)}
                                                            className="px-3 py-1 bg-[#CBE2D7] text-black rounded-md hover:bg-[#938A8A] transition-colors flex items-center gap-1"
                                                        >
                                                            <FiBarChart2 className="inline mr-1" />
                                                            Statistiques
