...
const Dashboard = ({ userType }) => {
    ...
    const handlePreviewFile = (filename) => {
        // Utilisez directement l'URL de l'API backend
        const fileUrl = `http://localhost:8081/api/files/${encodeURIComponent(filename)}`;
        
        // Pour les PDF, ouvrir dans un nouvel onglet
        if (filename.endsWith('.pdf')) {
          window.open(fileUrl, '_blank');
        } else {
          // Pour d'autres types de fichiers, utiliser axios
          axios.get(fileUrl)
            .then(response => {
