...
/*const handlePreviewFile = (fileUrl) => {
  ...
  <div className="space-y-4 h-[136px] overflow-y-auto pr-2"> {/* Hauteur calculée pour 2 activités */}
    {recentActivities.length > 0 ? (
      recentActivities.map((activity, index) => (
        <div key={index} className="flex items-center mb-2 min-h-[60px]"> {/* Hauteur fixe pour chaque activité */}
          <div className={`w-6 h-6 rounded-full mr-4 flex justify-center items-center ${
            ...
            activity.type === 'NEW_STUDENT_SIGNUP' ? 'bg-yellow-500' : // Nouvelle couleur pour les étudiants
            'bg-gray-400'
          }`}>
            <i className={`fas ${
              activity.type === 'NEW_TRAINER_TO_VALIDATE' ? 'fa-user-check' :
              activity.type === 'NEW_COURSE_TO_VALIDATE' ? 'fa-book' :
              activity.type === 'NEW_STUDENT_SIGNUP' ? 'fa-user-graduate' : // Nouvelle icône pour les étudiants
              'fa-info-circle'
            } text-white`} />
          </div>
          <div>
