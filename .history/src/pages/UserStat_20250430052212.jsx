...
const UserStat = () => {
    ...
    return (
        <div className="relative min-h-screen">
            <div className="fixed inset-0 z-[-1] opacity-10 bg-cover bg-center" style={{ backgroundImage: 'url(/images/Screenshot_2181.jpg)', backgroundSize: 'cover', backgroundPosition: 'center' }}/>
            <div className="bg-gray-50/80 min-h-screen py-8 relative z-10">
                <motion.div className="container mx-auto px-4" initial="hidden" animate="visible" variants={containerVariants}>
                    <motion.div className="relative rounded-xl shadow-lg p-6 mb-8 overflow-hidden border-2 border-[#FACC15]" variants={itemVariants}>
                        ...
                        <div className="absolute right-0 top-0 h-full w-1/3 overflow-hidden z-0">
                            <img src="/images/Subtract.png" alt="" className="h-full object-cover object-left" style={{ transform: 'translateX(130%)', opacity: 0.7 }}/>
                        </div>
                        <div className="relative z-10">
                            <div className="flex flex-col items-center text-center">
                                ...
                                <div className="mt-4 flex flex-wrap justify-center gap-4 text-white/90">
                                    ...
                                    <div className="bg-white/10 backdrop-blur-sm px-3 py-1 rounded-full">
                                        <span>{userType === 'FORMATEUR' ? `${stats?.etudiantsAbonnes} étudiants` : `${stats?.formateursSuivis} formateurs suivis`}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </motion.div>
                    <motion.div className="hidden md:grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8" variants={containerVariants}>
                        ...
                        <motion.div variants={itemVariants} className="bg-white p-6 rounded-xl shadow-lg">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-2xl font-semibold text-gray-800">{stats?.tauxValidation ? `${stats.tauxValidation}%` : 'N/A'}</p>
                                </div>
                            </div>
                        </motion.div>
                        <motion.div variants={itemVariants} className="bg-white p-6 rounded-xl shadow-lg">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-xl font-semibold text-gray-800">{stats?.dateDernierCours || 'Aucun'}</p>
                                </div>
                            </div>
                        </motion.div>
                        <motion.div variants={itemVariants} className="bg-white p-6 rounded-xl shadow-lg">
                                ...
                                <div className="flex-1">
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {stats?.coursParCategorie && Object.entries(stats.coursParCategorie).map(([categorie, count]) => (
                                            <span key={categorie} className="px-3 py-1 bg-gray-100 rounded-full text-sm">{categorie}: {count}</span>
                                        ))}
                                    </div>
                                </div>
                            </div>
                        ...
                        <motion.div variants={itemVariants} className="bg-white p-4 rounded-lg shadow">
                            <div className="flex items-center justify-between">
                                <div className="flex items-center space-x-3">
                                    ...
                                    <div>
                                        <h3 className="text-sm font-medium text-gray-500">Validation rate</h3>
                                        <p className="text-xl font-semibold">{stats?.tauxValidation ? `${stats.tauxValidation}%` : 'N/A'}</p>
                                    </div>
                                </div>
                            </div>
                        </motion.div>
                        <motion.div variants={itemVariants} className="bg-white p-4 rounded-lg shadow">
                            <div className="flex items-center justify-between">
                                <div className="flex items-center space-x-3">
                                    ...
                                    <div>
                                        <h3 className="text-sm font-medium text-gray-500">Most recent course</h3>
                                        <p className="text-lg font-semibold">{stats?.dateDernierCours || 'Aucun'}</p>
                                    </div>
                                </div>
                            </div>
                        </motion.div>
                        ...
                        <motion.div variants={itemVariants} className="bg-white p-4 rounded-lg shadow">
                            <div className="flex items-start space-x-3">
                                ...
                                <div className="flex-1">
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {stats?.coursParCategorie && Object.entries(stats.coursParCategorie).map(([categorie, count]) => (
                                            <span key={categorie} className="px-2 py-1 bg-gray-100 rounded-full text-xs">{categorie}: {count}</span>
                                        ))}
                                    </div>
                                </div>
                            </div>
                    ...
                    <div className="hidden md:grid grid-cols-1 lg:grid-cols-2 gap-8">
                        ...
                        <motion.div variants={itemVariants} className="lg:col-span-1 h-full">
                            <div className="bg-white rounded-xl shadow-lg p-6 h-full">
                                <h2 className="text-2xl font-semibold text-gray-800 mb-6">Distribution by field</h2>
                                <div className="h-80">
                                    <ResponsiveContainer width="100%" height="100%">
                                        <PieChart>
                                            <Pie data={formateurCharts?.pieData} cx="50%" cy="50%" labelLine={false} outerRadius={80} fill="#8884d8" dataKey="value" label={({ name, percent }) => `${name}: ${(percent * 100).toFixed(0)}%`}>
                    ...
                    <div className="md:hidden space-y-4 mb-8">
                        <motion.div variants={itemVariants}>
                            <div className="bg-white rounded-lg shadow p-4">
                                ...
                                <div className="h-64">
                                    <ResponsiveContainer width="100%" height="100%">
                                        <BarChart data={formateurCharts?.progressData} margin={{ top: 20, right: 20, left: 0, bottom: 20 }}>
                                            <CartesianGrid strokeDasharray="3 3"/>
                                            <XAxis dataKey="name"/>
                                            <YAxis/>
                                            <Tooltip/>
                        ...
                        <motion.div variants={itemVariants}>
                            <div className="bg-white rounded-lg shadow p-4">
                                ...
                                <div className="h-64">
                                    <ResponsiveContainer width="100%" height="100%">
                                        <PieChart>
                                            <Pie data={formateurCharts?.pieData} cx="50%" cy="50%" outerRadius={60} fill="#8884d8" dataKey="value" label={({ name, percent }) => `${name}: ${(percent * 100).toFixed(0)}%`}>
                                                {formateurCharts?.pieData?.map((entry, index) => (
                                                    <Cell key={`cell-${index}`} fill={DOMAIN_COLORS[index % DOMAIN_COLORS.length]}/>
                                                ))}
                                            </Pie>
